...
f = open('sample_resume_1')
input = f.read()

# asdf = re.findall('([A-Z][a-z]+|\.)(?:\s+([A-Z][a-z]+|\.))*(?:\s+[a-z][a-z\-]+){0,2}\s+([A-Z][a-z]+|\.)', input)
# for match in asdf:
# 	print(' '.join(match).replace('  ', ' '))

match = re.search('([A-Z][a-z]+|\.)(?:\s+([A-Z][a-z]+|\.))*(?:\s+[a-z][a-z\-]+){0,2}\s+([A-Z][a-z]+|\.)',
                  input)
...
match = re.search('Education(\n.*?)+((Bachelor|Masters|Doctorate).*)', input, re.I)
if match is not None:
